...
Protobuf是由谷歌开源的一套结构化的数据存储方案，类似于XML、Json. 
...
```
可以用protoc编译器将".proto"文件编译成不同编程语言的类文件（如 C++、Java、Python 等），这些类可以用于在代码中创建、序列化和反序列化 Protobuf 消息。

### Protobuf优点
1. Protobuf序列化的数据是二进制格式，比json和xml小，所以传输，解析速度快
2. 类型安全
3. 格式简洁
...
# CAP理论
CAP理论分布式系统领域中的一个基本理论，用于描述在分布式数据存储系统中三个核心属性之间的权衡关系，三个属性如下：

* Consistency：所有节点在同一时间看到的数据是相同的

* Availability：系统始终能够响应读或写请求，即使某些节点发生故障或网络分区存在

* Partition Tolerance：系统能够在网络分区的情况下继续运行，即使网络的某部分出现了故障，导致部分节点无法通信

CAP理论指出，在一个系统中，**不可能同时完全满足**上述三个属性，只能在以下两种属性之间做出选择：

* CA：保证一致性和可用性，但如果发生网络分区，系统可能无法继续运行（即不具备分区容忍性）

*网络分区：分布式系统中的一些节点因网络故障或其他原因无法互相通信。这种情况下，系统中的不同部分可能无法同步数据，导致一致性问题。*

* CP：保证一致性和分区容忍性，但在网络分区时，系统可能会牺牲部分可用性（例如拒绝请求）

* AP：保证可用性和分区容忍性，但可能牺牲一致性（即在网络分区期间，节点可能会返回不一致的数据）。

在分布式的场景下，P属性必须选择，因为网络本身不是100%可靠的，只能是CP或者AP架构,C和A的权衡需要一定的机制，例如Quorum机制


# Quorum机制
分布式系统中，读写操作通过使用Quorum来确保数据一致性。

写操作被认为成功的前提是至少有 W 个节点确认了写入（W 是写 Quorum 的大小）；

读操作被认为成功的前提是至少有 R 个节点返回了读数据（R 是读 Quorum 的大小）。

要确保一致性，需要满足：W + R > N，N为总的副本数

例如有5个副本，写操作成功写入了3个，另外2个副本仍是旧数据，此时在读取的时候，只要确保读取副本的数量大于2，那么肯定就会读到最新的数据。

# 一致性哈希算法（Consistent Hashing）
是一种用于分布式系统中的哈希算法，它特别适合在动态变化的环境中（如节点的频繁增加或删除）保持数据的分布均匀性和系统的平稳运行。

设计目标：尽量减少由于节点的加入或退出对系统的影响，从而减少数据重新分配的数量
...
这么做的好处就是，当增加或者删除一台服务器时，受影响的数据仅仅是新添加或删除的服务器到其环空间中前一台的服务器（也就是顺着逆时针方向遇到的第一台服务器）之间的数据，其他都不会受到影响
...
## Gossip协议
在分布式系统中，Gossip协议用来保证网络中所有节点的数据一致性
